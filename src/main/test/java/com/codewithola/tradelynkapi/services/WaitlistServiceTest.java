...
class WaitlistServiceTest {
    ...
    void testAddToWaitlist_Success_NewEmail() {
        ...
        assertTrue(response.getSuccess() == null || !response.getIsDuplicate());
    ...
    @Test
    void testIsEmailOnWaitlist_NotExists() {
        // Arrange
        when(waitlistRepository.existsByEmailIgnoreCase("notfound@example.com"))
                .thenReturn(false);

        // Act
        boolean exists = waitlistService.isEmailOnWaitlist("notfound@example.com");

        // Assert
        assertFalse(exists);
        verify(waitlistRepository, times(1)).existsByEmailIgnoreCase("notfound@example.com");
    }

    @Test
    void testGetWaitlistEntry_Found() {
        // Arrange
        when(waitlistRepository.findByEmailIgnoreCase("test@example.com"))
                .thenReturn(Optional.of(savedWaitlist));

        // Act
        Optional<WaitlistResponse> response = waitlistService.getWaitlistEntry("test@example.com");

        // Assert
        assertTrue(response.isPresent());
        assertEquals("test@example.com", response.get().getEmail());
        assertEquals(testUUID, response.get().getId());
    }

    @Test
    void testGetWaitlistEntry_NotFound() {
        // Arrange
        when(waitlistRepository.findByEmailIgnoreCase("notfound@example.com"))
                .thenReturn(Optional.empty());

        // Act
        Optional<WaitlistResponse> response = waitlistService.getWaitlistEntry("notfound@example.com");
