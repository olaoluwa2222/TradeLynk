...
class WaitlistControllerIntegrationTest {
    ...
    @Test
    void testAddToWaitlist_MissingEmail_Returns400() throws Exception {
        // Arrange
        WaitlistRequest missingEmailRequest = WaitlistRequest.builder()
                .email("")
                .source("go.tradelynk.app")
                .build();

        String requestBody = objectMapper.writeValueAsString(missingEmailRequest);
    ...
    @Test
    void testCorsHeaders_FromAllowedOrigin() throws Exception {
        // Arrange
        WaitlistRequest request = WaitlistRequest.builder()
        ...
        // Act & Assert - CORS headers should be present
        mockMvc.perform(post("/api/v1/waitlist")
                .header("Origin", "https://go.tradelynk.app")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated());
    }
